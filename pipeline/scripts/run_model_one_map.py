...
class PatchesDataset(Dataset):
    def __init__(self, matrices_by_res, resolutions, normmat250, eps, image_size, horizontals, device):
    ...
    def __get_matrix(self, x_native_res, y_native_res, resolution, matrix_raw, matrix):
        ...
        return mat, valid_mat

    def __getitem__(self, idx):
        mat_list = []
        valid_mat_all = 1
        for resolution in self.resolutions:
            matrix, matrix_raw = self.matrices_by_res[str(resolution)]
            mat_norm, valid_mat = self.__get_matrix(self.horizontals[0][idx], self.horizontals[1][idx], resolution, 
                                        matrix_raw, matrix)
            valid_mat_all*=valid_mat
            mat = torch.from_numpy(mat_norm).reshape((1, self.image_size, self.image_size)).to(device=device, dtype=torch.float)
            mat_list.append(mat)
...
class WholeMatrixPredictor():
    ...
    def __init__(self, cooler_path , resolutions, image_size, step, normmats_path, device, result_name, models, labels_cutoff, batch_size = 512):
        self.resolutions = resolutions
        self.image_size = image_size
        self.step = step
        self.device = device
        ...
        for resolution, normmat_path in zip(resolutions, normmats_path):
    ...
    def _predict(self, horizontals, current_chr):
        matrices_by_res = {}
        for resolution in self.resolutions:
            matrix, matrix_raw = self.matrixes_list[str(resolution)][current_chr]
            matrices_by_res[str(resolution)] = (matrix, matrix_raw)
        dataset = PatchesDataset(matrices_by_res, self.resolutions, self.normmat250, self.eps, self.image_size, horizontals, self.device)
...
predictor = WholeMatrixPredictor(validate_cooler, resolutions, image_size, step, normmats_path_val, device, args.result_save_path, models, float(args.labels_cutoff))
