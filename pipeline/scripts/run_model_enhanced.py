...
class WholeMatrixPredictor():
    ...
    def __load_horizontals_from_csv(self):
        """
        Build the same {chr: [[x_coords], [y_coords]]} structure as before,
        but from a user-provided CSV of submatrix centers (chr,x,y in bp)
        instead of scanning the whole genome matrix.
        """
        coords_df = pd.read_csv(self.coords_csv)
        horizontals_by_chr = {}
        for chr_name, group in coords_df.groupby('chr'):
            horizontals_by_chr[chr_name] = np.stack([group['x'].to_numpy(), group['y'].to_numpy()])
    ...
    def __init__(self, cooler_path, clean_cooler_path , resolutions, image_size, step, normmats_path, normmats_clean_path, device, result_name, models, labels_cutoff, coords_csv, batch_size = 512):
        self.resolutions = resolutions
        self.image_size = image_size
        self.step = step
        self.device = device
        ...
        self.horizontals = self.__load_horizontals_from_csv()
...
parser.add_argument('coords_csv', type=str, help='Path to CSV file with submatrix centers to score, columns: chr,x,y (bp coordinates)')
...
predictor = WholeMatrixPredictor(validate_cooler, clean_validate_cooler, resolutions, image_size, step, normmats_path_val, normmats_clean_path_val, device, args.result_save_path, models, float(args.labels_cutoff), args.coords_csv)
