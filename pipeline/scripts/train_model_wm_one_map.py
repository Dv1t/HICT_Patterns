...
class TrainDataset(Dataset):
    def __init__(self, cooler_path_list, trans_csv_path_list, resolution, image_size, normmat_paths):
    ...
    def get_matrix(self, mat_raw, mat_bal, eps, normmat250, coarse_grain=False):
        if coarse_grain:
    ...
    def __getitem__(self, idx):
        ...
        mat_norm = self.get_matrix(mat_r, mat_b, eps, normmat250, True)
        if np.random.random() > 0.6:
                mat_norm = np.rot90(mat_norm, k=2)
        tens = torch.from_numpy(mat_norm.copy()).reshape((1, self.image_size, self.image_size)).to(device=device, dtype=torch.float)
