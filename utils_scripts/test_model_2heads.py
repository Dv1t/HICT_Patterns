...
class TrainDatasetDiagonal(Dataset):
    def __init__(self, cooler_path_list, trans_csv_path_list, resolution, image_size, clean_cooler_list, normmat_path, normmat_clean_path, detection=True, blur=True):
        ...
        normmat_bydist = np.exp(np.load(normmat_clean_path))[:image_size]
        normmat = normmat_bydist[np.abs(np.arange(image_size)[:, None] - np.arange(image_size)[None, :])]
        ...
        for trans_csv_path, cooler_path, clean_cooler_path, index in tqdm(zip(trans_csv_path_list, cooler_path_list, clean_cooler_list, range(len(trans_csv_path_list)))):
            ...
            for chr in c.chromnames:
                ...
                f_matrix = np.log10(matrix_raw+1e-6) - np.log10(matrix_clean+1e-6)

                matrixes_by_chr[chr] = (matrix, matrix_raw, f_matrix)
    ...
    def __getitem__(self, idx_d):
        ...
        mat_log_change = torch.from_numpy(matrix_full[2][x-pad:x+pad, y-pad:y+pad]).to(device=device, dtype=torch.float)

        mat = torch.from_numpy(mat_norm).to(device=device, dtype=torch.float)
        mat_double = torch.cat((mat, mat_log_change))
        try:
            tens = mat_double.reshape((2, self.image_size, self.image_size)).to(device=device, dtype=torch.float)
        except RuntimeError:
...
class Detect_2heads_Model(nn.Module):
    def __init__(self, in_channels=1, image_size=40, num_models=10):
        super(Detect_2heads_Model, self).__init__()
...
model = Detect_2heads_Model(in_channels=2, image_size=48, num_models=10)
model.to(device=device)
model.load_state_dict(torch.load(f'weights_2heads_normmat_logchange_raw/torch_ensemble_10k_48_diag.pt', map_location=device))
model.eval()
...
model = Detect_2heads_Model(in_channels=2, image_size=48, num_models=10)
model.to(device=device)
model.load_state_dict(torch.load(f'weights_2heads_normmat_logchange_raw/torch_ensemble_50k_48_diag.pt', map_location=device))
model.eval()
criterion = nn.CrossEntropyLoss()
num_epochs = 1
dataloaders = dict()
...
model = Detect_2heads_Model(in_channels=2, image_size=48, num_models=10)
model.to(device=device)
model.load_state_dict(torch.load(f'weights_2heads_normmat_logchange_raw/torch_ensemble_25k_48_diag.pt', map_location=device))
model.eval()
criterion = nn.CrossEntropyLoss()
num_epochs = 1
dataloaders = dict()
