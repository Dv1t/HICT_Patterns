...
model.load_state_dict(torch.load(f'weights_normmat/torch_ensemble_10k_48_diag.pt', map_location=device))
model.eval()
...
model = DetectModel(in_channels=1, image_size=48, num_models=10)
model.to(device=device)
model.load_state_dict(torch.load(f'weights_normmat/torch_ensemble_50k_48_diag.pt', map_location=device))
model.eval()
criterion = nn.CrossEntropyLoss()
num_epochs = 1
dataloaders = dict()
...
model = DetectModel(in_channels=1, image_size=48, num_models=10)
model.to(device=device)
model.load_state_dict(torch.load(f'weights_normmat/torch_ensemble_25k_48_diag.pt', map_location=device))
model.eval()
criterion = nn.CrossEntropyLoss()
num_epochs = 1
dataloaders = dict()
