...
class TrainDatasetPatches(Dataset):
    def __init__(self, cooler_path_list, trans_csv_path_list, resolution, image_size, clean_cooler_list, normmat_path, detection=True):
        sv_count = 0
        ...
        for trans_csv_path, cooler_path, clean_cooler_path,index in tqdm(zip(trans_csv_path_list, cooler_path_list, clean_cooler_list, range(len(trans_csv_path_list)))):
            ...
            for i, sv in sv_file.iterrows():
                ...
                chr_index = random.randint(0, len(c_clean.chromsizes)-2)
    ...
    def get_matrix(self, mat_bal, mat_raw, normmat250, eps, isCg, isLog=True):
            ...
            mat250 = np.nanmean(np.nanmean(np.reshape(mat_bal, (self.image_size, 4, self.image_size, 4)), axis=3), axis=1)
...
model.load_state_dict(torch.load(f'weights_normmat_whole_4fold/torch_ensemble_10k_48.pt', map_location=device))
model.eval()
...
model = DetectModel(in_channels=1, image_size=48, num_models=10)
model.to(device=device)
model.load_state_dict(torch.load(f'weights_normmat_whole_4fold/torch_ensemble_50k_48.pt', map_location=device))
model.eval()
criterion = nn.CrossEntropyLoss()
num_epochs = 1
dataloaders = dict()
...
model = DetectModel(in_channels=1, image_size=48, num_models=10)
model.to(device=device)
model.load_state_dict(torch.load(f'weights_normmat_whole_4fold/torch_ensemble_25k_48.pt', map_location=device))
model.eval()
criterion = nn.CrossEntropyLoss()
num_epochs = 1
dataloaders = dict()
