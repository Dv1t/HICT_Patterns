...
class TrainDatasetDiagonal(Dataset):
    ...
    def __getitem__(self, idx_d):
        ...
        if row.is_sv and self.images_to_save > 0:
            ...
            fig.colorbar(im)
            plt.savefig(f'saved_matrices_log_change/test_after_log_change_{self.resolution//1000}_{self.images_to_save}_{sv_info.chr}_{x}_{y}_{shift}.png')
            plt.close()
...
model.load_state_dict(torch.load(f'weights_log_change/torch_ensemble_10k_48_diag.pt', map_location=device))
model.eval()
...
model = DetectModel(in_channels=1, image_size=48, num_models=10)
model.to(device=device)
model.load_state_dict(torch.load(f'weights_log_change/torch_ensemble_50k_48_diag.pt', map_location=device))
model.eval()
criterion = nn.BCELoss()
num_epochs = 1
dataloaders = dict()
...
model = DetectModel(in_channels=1, image_size=48, num_models=10)
model.to(device=device)
model.load_state_dict(torch.load(f'weights_log_change/torch_ensemble_25k_48_diag.pt', map_location=device))
model.eval()
criterion = nn.BCELoss()
num_epochs = 1
dataloaders = dict()
