...
class TrainDatasetDiagonal(Dataset):
    def __init__(self, cooler_path_list, trans_csv_path_list, resolution, image_size, normmat_path, validate=False):
        ...
        for trans_csv_path, cooler_path, index in tqdm(zip(trans_csv_path_list, cooler_path_list, range(len(trans_csv_path_list)))):
            ...
            for chr in c.chromnames:
                ...
                matrixes_by_chr[chr] = (matrix_raw, matrix)
    ...
    def get_matrix(self, mat_raw, mat_bal, coarse_grain=False):
            if coarse_grain:
    ...
    def __getitem__(self, idx_d):
        ...
        mat_norm = self.get_matrix(mat_r, mat_b,  True)
...
train_model(dataloaders, model, f'log_25kb_{cfg.model_name[0]}', criterion, optimizer, device, num_epochs, phases= ['train', 'test'], model_name=cfg.model_name[0])
torch.save(model.state_dict(), f'weights_{cfg.model_name[0]}/normmat_cg_48_diag_25Kbp.pt')
print(f'Trained 25kb {cfg.model_name[0]} with config {args.cfg_path}')