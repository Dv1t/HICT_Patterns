...
class TrainDatasetDiagonal(Dataset):
    ...
    def __getitem__(self, idx_d):
        ...
        if row.is_sv and self.images_to_save > 0:
            os.makedirs('saved_matrices_normmat', exist_ok=True)
            fig = plt.figure()
            ax = fig.add_subplot(111)
            im = ax.matshow(mat_norm, cmap='bwr')
            fig.colorbar(im)
            plt.savefig(f'saved_matrices_normmat/after_normmat_{self.resolution//1000}_{self.images_to_save}_{sv_info.chr}_{x}_{y}_{shift}.png')
            plt.close()
            fig = plt.figure()
            ax = fig.add_subplot(111)
            im = ax.matshow(mat_r, cmap='bwr')
            fig.colorbar(im)
            plt.savefig(f'saved_matrices_normmat/raw_mat_{self.resolution//1000}_{self.images_to_save}_{sv_info.chr}_{x}_{y}_{shift}.png')
            plt.close()
...
train_model(dataloaders, model, 'log_normmat_25', criterion, optimizer, device, num_epochs, phases= ['train', 'test'])
...
train_model(dataloaders, model, 'log_normmat_50', criterion, optimizer, device, num_epochs, phases= ['train', 'test'])
...
train_model(dataloaders, model, 'log_normmat_10', criterion, optimizer, device, num_epochs, phases= ['train', 'test'])
