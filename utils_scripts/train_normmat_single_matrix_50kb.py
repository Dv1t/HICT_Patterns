...
train_model(dataloaders, model, f'log_50kb_{cfg.model_name[0]}', criterion, optimizer, device, num_epochs, phases= ['train', 'test'], model_name=cfg.model_name[0])
torch.save(model.state_dict(), f'weights_{cfg.model_name[0]}/normmat_cg_48_diag_50Kbp.pt')
print(f'Trained 50kb {cfg.model_name[0]} with config {args.cfg_path}')